...
export async function POST(request: Request) {
  try {
    ...
    const currentState = await getAutoTempState(containerId);
    const updates: any = {};
    ...
    const newState = await updateAutoTempState(containerId, updates);

    // Log toggle changes
    if (enabled !== undefined && enabled !== currentState.enabled) {
      await addLog(containerId, `Automation settings modified. System is now ${enabled ? 'Active' : 'Deactivated'}.`);
    }
    
    // Log alert toggle changes
    if (alertsEnabled !== undefined && alertsEnabled !== currentState.alertsEnabled) {
      await addLog(containerId, `Alert notifications system toggled ${alertsEnabled ? 'ON' : 'OFF'}.`);
    }
    
    // Log alert settings changes
    if (alertEmail !== undefined && alertEmail !== currentState.alertEmail) {
      if (alertEmail) {
        await addLog(containerId, `Alert notification email set to: ${alertEmail}`);
      } else {
        await addLog(containerId, `Alert email cleared (email notifications deactivated).`);
      }
    }
    
    if (criticalLowTemp !== undefined && criticalLowTemp !== currentState.criticalLowTemp) {
      await addLog(containerId, `Critical low temperature safety limit adjusted to: ${criticalLowTemp}°C`);
    }
